...
def modal_servico(modo, cliente_id, carro_id, servico_atual=None):
    ...
    for indice, registro in enumerate(registros_atuais):
        ...
        col_total.markdown("<div style='height: 0.1rem;'></div>", unsafe_allow_html=True)
        col_total.markdown(
            f"<div style='padding: 0.34rem 0.75rem; border: 2px solid #d1d5db; font-weight: 600; line-height: 1.2;'>{formatar_moeda(total)}</div>",
            unsafe_allow_html=True,
        )
